...
Nested folders compound the problem, pushing paths past OS limits. This tool rewrites the zip with clean names:
...
There are two ways to use it: a **web app** (no installation) and a **command-line Python script**.

---

## Web app

**[michael-baker-content.github.io/notion-clean/](https://michael-baker-content.github.io/notion-clean/)**

1. Open the link above
2. Drop your Notion export `.zip` onto the page, or click to browse for it
3. A preview shows every file that will be renamed
4. Edit the output filename if you want (it defaults to your source file's name with `-cleaned` appended)
5. Click **Download cleaned zip**

Everything runs in your browser — your files are never sent to a server.

---

## Python script

`notion_clean.py` is a single-file command-line tool with no dependencies beyond Python's standard library. Use it when you want to automate cleaning, script it into a larger workflow, or process files without opening a browser.
...
Both the web app and the Python script share the same logic and handle the same cases.
...
**Double-wrapped zips** — Notion sometimes wraps the real archive inside an outer zip (the outer zip's name carries its own UUID). Both tools detect this automatically and process the inner archive.
...
The same regex change applies to the web app's `ID_RE` constant in `index.html`.
